...
PR #34's squash removed all five briefs' branch-commit pins and turned `main`
CI red (hotfixed in #35). When a pin is unreachable from HEAD, whether it is
absent in a fresh clone or retained as an unreachable object in a stale clone,
`validate_repository_truth` uses the last commit that touched the brief. That
commit contains the brief and its cited evidence together, so later edits to
cited files still trigger drift checks. The fallback skips only the
`checked_at` comparison because that date belongs to the missing pin, and it
prints a non-failing NOTE naming the substitute commit so the rot stays
visible. Briefs without committed history still fail closed. Self-tests cover
a passing fallback, evidence validation on the fallback, the no-history
failure, and a reachable-pin-stays-authoritative lock (a regression routing
good pins through the fallback trips it). Both the evergreen
`GIF-PRODUCTION-PLAYBOOK.md` and the batch record recommend pins already on
`main`.
