...
- `automations.yaml`: example automations. The first one forwards a SleepRadar
  connection problem to a notify service, which is the only one of these that
  tells you when the app stops; it needs your notify service and no occupancy
  gate, because it acts on the diagnostic entity rather than on sleep data. The
  rest act on the sleep data (dim lights while asleep, hold the vacuum, cool
  down for deep sleep) and need your own light/vacuum/thermostat entity IDs and
  the occupancy-gated helpers.
...
- `recorder.yaml`: example Recorder include for long-term history. Pre-wired,
  ...
  past gap can still be explained after the fact.
